...
2. 數值變動需轉化為「可感知的體驗描寫」（如：脈搏加速、代碼在視網膜閃爍）。若 HP 或 SP 偏低，需在敘事中呈現瀕死或力竭的絕境感。
3. 使用 Markdown 分段。禁止出現 meta、系統數據等技術資訊。
4. 必須強烈推進劇情，嚴禁重複上一章節的內容或原地踏步。
5. 必須針對『玩家當前行動』給出明確且深刻的因果回饋，展現玩家行動的影響力。`;
...
3. 選項 3-5 個，禁句號，每項 <20 字。選項必須基於當前局勢給出。若 HP 或 SP 極低(<=20)或歸零，務必提供相關的應對手段(如尋找靈藥、打坐休息等)或生存抉擇。
4. 數值範圍 -30 ~ +30 整數。注意系統中 HP 與 SP 範圍為 0~100，若達極值請調整數值變動與後續發展，避免給出無效變動。`;
...
描述：${scene?.description || ''}`;
  ...
  content += `\n\n【玩家狀態】
HP ${g.player.hp}/100, SP ${g.player.sp}/100, 威脅 ${g.player.threat}, 能力 ${JSON.stringify(g.player.abilities)}`;

  content += `\n\n【歷史紀錄 (前情提要，請接續發展勿重複敘述)】
${g.history?.slice(-3).map(h => `- 玩家行動: ${h.action}\n- 發生事件: ${h.result?.narrative.slice(0, 100)}...`).join('\n') || '無'}`;

  content += `\n\n【玩家當前行動 (你的敘事必須以此為核心，並給出直接後果)】
${action}`;
